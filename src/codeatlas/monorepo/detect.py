...
def detect_subprojects(root: Path, excludes: Iterable[str]) -> list[SubProject]:
    ...
    `declared_deps` (source des arêtes `service_dep`).
    """
    ...
    subprojects: dict[str, tuple[str, Path, str]] = {}
    for rel_dir, (language, manifest) in found.items():
        sub_id = "main" if rel_dir == "." else rel_dir.replace("/", "-")
        subprojects[sub_id] = (language, manifest, rel_dir)
        name = _project_name(manifest, language)
        if name:
            names[name] = sub_id

    results = []
    for sub_id, (language, manifest, rel_dir) in subprojects.items():
        declared = tuple(
            sorted(
                names[dep]
